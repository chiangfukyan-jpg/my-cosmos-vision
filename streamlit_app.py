...
from core_logic import scan_dragon_logic, smart_fetch, check_stop_loss, AssetRanker
import time
import os
import json
    ...
    if run_normal or run_hunt:
        ...
        if target_category == "📦 美股 ETF (~360隻)":
            ...
            try:
                ...
                st.warning("⚠️ 找不到 Market_Focus.csv，爺爺將自動為你載入備用預設美股名單。")
            ...
            try:
                ...
                st.warning("⚠️ 找不到 SP500_Equities.csv，爺爺將自動為你載入備用預設美股名單。")
                for sub_list in US_STOCK_MAP.values(): current_tickers.extend(sub_list)
                current_tickers = list(set(current_tickers))
        ...
        if current_tickers:
            st.info(f"🚀 啟動超級運算引擎！共掃描 {len(current_tickers)} 隻資產...")
            with st.spinner("爺爺的高頻雷達正在為你計算資金流與排名..."):
                try:
                    ranker = AssetRanker(tickers=current_tickers, lookback=lookback_days)
                    
                    if run_hunt:
                        result_df = ranker.run(mode="hunt")
                    else:
                        result_df = ranker.run(mode="normal")
                    
                    if result_df is not None and not result_df.empty:
                        st.success("✅ 運算完成！究極龍虎榜已生成！")
                        st.dataframe(result_df, use_container_width=True, height=800)
                    else:
                        st.warning("💤 該戰區目前無符合條件之強勢股。")
                except Exception as e:
                    st.error(f"❌ 運算過程中發生錯誤: {e}")
        else:
            st.error("⚠️ 無法獲取股票名單，請檢查網絡或檔案是否正確！")

# ==========================================
# 💰 模式三：大戶資金流透視 (福德金字塔)
# ==========================================
elif operation_mode == "💰 大戶資金流透視 (福德金字塔)":
    st.markdown("<h1 style='text-align:center; color:#FF9900;'>💰 大戶資金流透視 (福德金字塔)</h1>", unsafe_allow_html=True)
    st.info("🚧 爺爺正在為你升級此模組，下一個版本將完整釋出福德金字塔的資金流演算法，敬請期待！")
